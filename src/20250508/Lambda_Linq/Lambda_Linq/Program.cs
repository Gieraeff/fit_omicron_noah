...
namespace Lambda_Linq
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            doSomething("Chimpansini bananini");

            // Introduction to lambda
            doSomething = (string msg) => { Console.WriteLine(msg); };
            ...
            //example
            var valueList = new List<int> {5, 8, 10, 2, 15, 7, 20, 29 };
            ...
            //greaterThen10Values = Filter(valueList, GreaterThen8);

            // LINQ
            var result = valueList.Where(x => x > 5)
                                  .Select(x => 2 * x);
        }
